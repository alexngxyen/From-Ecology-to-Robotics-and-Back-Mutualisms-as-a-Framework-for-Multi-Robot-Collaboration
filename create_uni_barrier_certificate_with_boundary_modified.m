function [ uni_barrier_certificate ] = create_uni_barrier_certificate_with_boundary_modified(varargin)
...
%         num_constraints = (num_disturbs^2)*temp + num_robots*num_obstacles*num_disturbs + num_robots;
        ...
        wheel_vel_limit_modified = [wheel_vel_limit(1)*ones(2,1); wheel_vel_limit(2)*ones(2,1)];
        vnew = quadprog(H, double(f), -A(1:num_constraints,1:2*num_robots), -b(1:num_constraints), [], [], -wheel_vel_limit_modified, wheel_vel_limit_modified, [], opts);
        %disp('f')
        %f(1:4)
