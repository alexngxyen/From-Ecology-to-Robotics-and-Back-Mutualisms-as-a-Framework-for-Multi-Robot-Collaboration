%% Ecobotics Mutualism Task Completion (Landscape 4 - Very Clumped)
%
...
% environment with a very clumped pixel distribution. The robots 
% traverse straight line paths. Additionally, the leader switches require a
...
while(~init_checker(x, final_goal_points))
    ...
    if ~(domainMask(idx(1, 2), idx(2, 2)) == 0 && x(1, 2) <= 0 && x(2, 2) >= 0)
        ...
        elseif flagLeader == 2
            ...
            % turtle (leader) in control
             dxi = [k1*(ordered_target_states(count_target, :)' - x(1:2, 1)) - [0;0.15], ...                                 
                    k2*(x(1:2, 1) - x(1:2, 2))];    
    ...
    elseif (domainMask(idx(1, 2), idx(2, 2)) == 0 && x(1, 2) <= 0 && x(2, 2) >= 0)
        if count_collab == 1
...
% Print Results
fprintf('\nMutualism Tasks (Landscape 4 - Very Clumped)\n');
