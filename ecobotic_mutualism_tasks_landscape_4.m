%% Ecobotics Mutualism Task Completion (Landscape 4 - Very Low-Variability)
%
...
% environment with a very low-variability landscape profile. The transitions
...
columns    = 2;
if strcmp(flag_configuration, 'A')
    domainMask = -1*[zeros(n_grid/rows, (1.6/columns)*n_grid), ones(n_grid/rows, (1.6/columns)*n_grid); ...
                     ones(n_grid/rows, (1.6/columns)*n_grid), zeros(n_grid/rows, (1.6/columns)*n_grid)];
elseif strcmp(flag_configuration, 'B')
    domainMask = -1*[ones(n_grid/rows, (1.6/columns)*n_grid), zeros(n_grid/rows, (1.6/columns)*n_grid); ...
                     zeros(n_grid/rows, (1.6/columns)*n_grid), ones(n_grid/rows, (1.6/columns)*n_grid)];
end
...
while(~init_checker(x, final_goal_points))
    ...
    if strcmp(flag_configuration, 'A')
        collabConditions = (domainMask(idx(1, 2), idx(2, 2)) == 0 && x(1, 2) <= 0 && x(2, 2) >= 0);
    elseif strcmp(flag_configuration, 'B')
        collabConditions = (domainMask(idx(1, 2), idx(2, 2)) == -1 && x(1, 2) <= -0.75 && x(2, 2) >= 0.35);
    end
    ...
    if ~collabConditions
        ...
        if flagLeader == 1
            ...
            dxi = [k1*(x(1:2, 2) - x(1:2, 1)), ...
                   k2*(ordered_target_states(count_target, :)' - x(1:2, 2)) - 0*[0;0.15]];  
        ...
        elseif flagLeader == 2
            ...
            % turtle (leader) in control
             dxi = [k1*(ordered_target_states(count_target, :)' - x(1:2, 1)) - 0*[0;0.15], ...                                 
                    k2*(x(1:2, 1) - x(1:2, 2))];    
...
E_r_switch = switch_cost*1;  % switching cost for rabbit [J]
...
E_t_switch = switch_cost*1;  % switching cost for turtle [J]
...
% Print Results
fprintf('\nMutualism Tasks (Landscape 4 - Very Low-Variability)\n');
