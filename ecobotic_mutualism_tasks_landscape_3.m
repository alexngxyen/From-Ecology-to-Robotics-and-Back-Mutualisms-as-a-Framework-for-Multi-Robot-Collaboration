%% Ecobotics Mutualism Task Completion (Landscape 3 - Low-Variability)
%
...
% environment with a low-variability landscape profile. The transitions
...
elseif strcmp(flag_configuration, 'B')             
    domainMask = -1*[ones(n_grid/rows, (1.6/columns)*n_grid), zeros(n_grid/rows, (1.6/columns)*n_grid), ones(n_grid/rows, (1.6/columns)*n_grid); ...
                     zeros(n_grid/rows, (1.6/columns)*n_grid), ones(n_grid/rows, (1.6/columns)*n_grid), zeros(n_grid/rows, (1.6/columns)*n_grid)]; 
end
...
while(~init_checker(x, final_goal_points))
    ...
    if strcmp(flag_configuration, 'A')
       collabConditions = (domainMask(idx(1, 2), idx(2, 2)) == 0 && x(1, 2) <= -0.5 && x(2, 2) >= 0);
    elseif strcmp(flag_configuration, 'B')
       collabConditions = (domainMask(idx(1, 2), idx(2, 2)) == -1 && x(1, 2) <= -0.5 && x(2, 2) >= 0);
    end
...
% Print Results
fprintf('\nMutualism Tasks (Landscape 3 - Low-Variability)\n');
