%% Ecobotics Mutualism Task Completion (Landscape 1 - Very High-Variability)
%
...
% environment with a very high-variability landscape profile. The transitions 
% between ``modes'' require a coordinated maneuver between the two robots. 
%
...
% Flags
flag_configuration = 'B';     % 'A': landscape configuration 1; 'B': landscape configuration 2 
...
elseif flag_IC == 0     
    if strcmp(flag_configuration, 'A')
        initial_positions = [ 1.15, 1.44 ; ...
                              0.17, 0.17 ; ...
                                 0,    0];
    elseif strcmp(flag_configuration, 'B')
        initial_positions = [ 1.44, 1.15 ; ...
                              0.17, 0.17 ; ...
                                 0,    0];
    end
...
% Initialize Leader Robot (`1': rabbit; `2' turtle) 
if strcmp(flag_configuration, 'A')
    flagLeader = 2;
elseif strcmp(flag_configuration, 'B')
    flagLeader = 1;
...
% Plot high-variability Pixel Distribution
...
columns    = 4;
if strcmp(flag_configuration, 'A')
    domainMask = -1*repmat([repmat([zeros(n_grid/rows, (1.6/columns)*n_grid), ones(n_grid/rows, (1.6/columns)*n_grid)], [1, ceil(columns/2)]); ...
                            repmat([ones(n_grid/rows, (1.6/columns)*n_grid), zeros(n_grid/rows, (1.6/columns)*n_grid)], [1, ceil(columns/2)])], [rows/2, 1]); 
elseif strcmp(flag_configuration, 'B')
    domainMask = -1*repmat([repmat([ones(n_grid/rows, (1.6/columns)*n_grid), zeros(n_grid/rows, (1.6/columns)*n_grid)], [1, ceil(columns/2)]); ...
                            repmat([zeros(n_grid/rows, (1.6/columns)*n_grid), ones(n_grid/rows, (1.6/columns)*n_grid)], [1, ceil(columns/2)])], [rows/2, 1]); 
end 
...
custom_colormap = [0, 0, 1;        % Blue
                   0.7, 0.4, 0.1]; % Brown
domain_img = image(flip(x_img), flip(y_img), domainMask,'CDataMapping','scaled');
...
if flag_mp4_video == 1
    ...
    video_filename = 'ecobotic_mutualism_tasks_very_uniform.mp4';
...
%     iteration_caption = sprintf('Iteration %d', 0);
    ...
    environment = sprintf('Very high-variability Pixel Distribution');
...
while(~init_checker(x, final_goal_points))
    ...
    if strcmp(flag_configuration, 'A')
        collabConditions = (domainMask(idx(1, 2), idx(2, 2)) == 0 && x(1, 2) <= -0.75 && x(2, 2) >= 0.45);
    elseif strcmp(flag_configuration, 'B')
        collabConditions = (domainMask(idx(1, 2), idx(2, 2)) == -1 && x(1, 2) <= -0.75 && x(2, 2) >= 0.45);
    end
...
if flag_save_workspace == 1
    save('ecobotic_mutualism_tasks_very_uniform_logs.mat');
end
...
E_r_switch = switch_cost*4;  % switching cost for rabbit [J]
...
E_t_switch = switch_cost*4;  % switching cost for turtle [J]
...
% Print Results
fprintf('\nMutualism Tasks (Landscape 1 - Very High-Variability)\n');
