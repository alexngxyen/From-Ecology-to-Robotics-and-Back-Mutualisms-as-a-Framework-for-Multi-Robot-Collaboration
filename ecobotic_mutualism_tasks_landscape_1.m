%% Ecobotics Mutualism Task Completion (Landscape 1 - Very Uniform)
%
...
% environment with a very uniform pixel distribution. The robots traverse 
...
%     iteration_caption = sprintf('Iteration %d', 0);
    ...
    environment = sprintf('Very Uniform Pixel Distribution');
...
while(~init_checker(x, final_goal_points))
    ...
    if ~(domainMask(idx(1, 2), idx(2, 2)) == 0 && x(1, 2) <= -0.75 && x(2, 2) >= 0.45)
    ...
    elseif (domainMask(idx(1, 2), idx(2, 2)) == 0 && x(1, 2) <= -0.75 && x(2, 2) >= 0.45)
        if count_collab == 1
...
% Print Results
fprintf('\nMutualism Tasks (Landscape 1 - Very Uniform)\n');
